...
class ExactShippingModel:
    ...
    seller_shipping_cost_vars: dict[str, object]
    seller_shipping_weight_trigger_vars: dict[
        str, list[tuple[shipping.ShippingPriceStep, object]]
    ]
    seller_shipping_value_trigger_vars: dict[
        str, list[tuple[shipping.ShippingPriceStep, object]]
    ]
...
def _seller_shipping_cost_expr(
    *,
    ...
    buyer_country: str,
    route_book: shipping.ShippingRouteBook,
    shipping_cost_vars: dict[str, object] | None = None,
    weight_trigger_vars: dict[str, list[tuple[shipping.ShippingPriceStep, object]]]
    | None = None,
    value_trigger_vars: dict[str, list[tuple[shipping.ShippingPriceStep, object]]]
    | None = None,
):
    route_key = (
    ...
    if shipping_cost_vars is not None:
        shipping_cost_vars[seller_id] = shipping_cost_var
    ...
    for index, step in enumerate(route_approximation.weight_steps):
        ...
        seller_weight_triggers.append((step, trigger_var))
        model.Add(
    ...
    if weight_trigger_vars is not None:
        weight_trigger_vars[seller_id] = seller_weight_triggers

    seller_value_triggers = []
    for index, step in enumerate(route_approximation.value_steps):
        trigger_var = model.NewBoolVar(f"ship_value_{seller_id}_{index}")
        seller_value_triggers.append((step, trigger_var))
    ...
    if value_trigger_vars is not None:
        value_trigger_vars[seller_id] = seller_value_triggers
...
def _cheapest_warm_start_shipping_option(
    *,
    seller_country: str,
    buyer_country: str,
    route_book: shipping.ShippingRouteBook,
    ...
    if route_approximation is not None:
        return shipping.ShippingTier(
            total_price_cents=route_approximation.base_price_cents,
            max_value_cents=route_approximation.base_max_value_cents,
            max_weight_grams=route_approximation.base_max_weight_grams,
        )
    ...
    return _cheapest_shipping_tier(route_tiers.tiers)
...
def _build_exact_shipping_model(
    *,
    ...
    seller_shipping_cost_vars = {}
    seller_shipping_weight_trigger_vars = {}
    seller_shipping_value_trigger_vars = {}
    ...
    for seller_id, offers in seller_offers.items():
        ...
        objective_terms.append(
            _seller_shipping_cost_expr(
                ...
                route_book=route_book,
                shipping_cost_vars=seller_shipping_cost_vars,
                weight_trigger_vars=seller_shipping_weight_trigger_vars,
                value_trigger_vars=seller_shipping_value_trigger_vars,
            )
        )
    ...
    return ExactShippingModel(
        ...
        seller_shipping_cost_vars=seller_shipping_cost_vars,
        seller_shipping_weight_trigger_vars=seller_shipping_weight_trigger_vars,
        seller_shipping_value_trigger_vars=seller_shipping_value_trigger_vars,
...
def _apply_exact_shipping_hints(
    *,
    ...
    for seller_id, shipping_cost_var in exact_model.seller_shipping_cost_vars.items():
        seller_selection = [
        ...
        if not seller_selection:
            exact_model.model.AddHint(shipping_cost_var, 0)
            for _, trigger_var in exact_model.seller_shipping_weight_trigger_vars.get(
                seller_id, []
            ):
                exact_model.model.AddHint(trigger_var, 0)
            for _, trigger_var in exact_model.seller_shipping_value_trigger_vars.get(
                seller_id, []
            ):
                exact_model.model.AddHint(trigger_var, 0)
            continue
        ...
        shipping_cost_cents = shipping.approximate_shipping_cost_cents(
            seller_country=seller_map[seller_id].country,
            buyer_country=buyer_country,
            total_value_cents=total_value_cents,
            total_weight_grams=total_weight_grams,
        ...
        exact_model.model.AddHint(shipping_cost_var, shipping_cost_cents)

        for step, trigger_var in exact_model.seller_shipping_weight_trigger_vars.get(
            seller_id, []
        ):
            exact_model.model.AddHint(
                trigger_var,
                1 if total_weight_grams > step.threshold_cents_or_grams else 0,
            )

        for step, trigger_var in exact_model.seller_shipping_value_trigger_vars.get(
            seller_id, []
        ):
            exact_model.model.AddHint(
                trigger_var,
                1 if total_value_cents > step.threshold_cents_or_grams else 0,
            )
...
def _solve_lowest_tier_warm_start(
    *,
    ...
    for seller_id, offers in seller_offers.items():
        ...
        cheapest_option = _cheapest_warm_start_shipping_option(
            seller_country=seller_map[seller_id].country,
            buyer_country=request.buyer_country,
            route_book=route_book,
        )
        ...
        # model.Add(total_units <= _max_units_for_tier(cheapest_option))

        objective_terms.append(cheapest_option.total_price_cents * active)
